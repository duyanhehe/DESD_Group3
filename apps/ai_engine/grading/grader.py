...
class QualityGrader:
    ...
    def analyze_defects(self, img_no_bg, hsv_img, fruit_mask, profile, fruit_type):
        ...
        if "ignore_zone" in profile:
            ...
            ignore_mask = cv2.inRange(hsv_img, profile["ignore_zone"]["lower"], profile["ignore_zone"]["upper"])
    ...
    def process_fruit(self, cropped_img, yolo_class_name):
        ...
        parts = yolo_class_name.split("_")

        fruit_type = parts[0] # e.g. "Apple"

        condition = parts[1]  # "Healthy" or "Rotten"
        ...
        if h < 30 or w < 30:
            ...
            return "Unanalyzable", 0.0, "Too Small", "Object is too small to analyze accurately."
        ...
        defect_pct, highlighted_img, _ = self.analyze_defects(img_no_bg, hsv_img, fruit_mask, defect_profile, fruit_type)
        ...
        if condition == "Healthy":
            # BRANCH 1: HEALTHY FRUIT -> GRADE BASED ON SCORING
            if size_score >= 80 and shape_score >= 90 and ripeness_pct >= 80:
                grade = "Grade A"
                explain_log.append("Premium Quality: Excellent size, shape, and ripeness.")
            elif size_score >= 60 and shape_score >= 85 and ripeness_pct >= 60:
                grade = "Grade B"
                explain_log.append("Standard Quality: Acceptable parameters.")
            elif size_score >= 40 and shape_score >= 80:
                grade = "Grade C"
                explain_log.append("Sub-standard Quality: Low ripeness or size.")
            else:
                grade = "Grade D"
                explain_log.append("Low Quality: Small size or irregular shape.")
            ...
            if defect_pct > 5.0 or edge_density >= density_threshold:
                explain_log.append(f"System Note: CV algorithm flagged {defect_pct}% area as potential anomaly (likely shadows/glare), but trusting YOLO 'Healthy' classification.")
            ...
            explain_log.append(f"Discarded: YOLO classified as Rotten. CV analysis verified defect area.")
